...
// Allocate Context if needed and set/reset common parameters. Returns 0 on success, error code otherwise.
func (s *Context) SwrAllocSetOpts(ocl int, osf AvSampleFormat, osr int, icl int, isf AvSampleFormat, isr, lo, lc int) int {
	return int(C.swr_alloc_set_opts2(
		(**C.struct_SwrContext)(unsafe.Pointer(&s)),
		(*C.struct_AVChannelLayout)(unsafe.Pointer(&C.struct_AVChannelLayout{order: 0, nb_channels: C.int(ocl)})),
		(C.enum_AVSampleFormat)(osf),
		C.int(osr),
		(*C.struct_AVChannelLayout)(unsafe.Pointer(&C.struct_AVChannelLayout{order: 0, nb_channels: C.int(icl)})),
		(C.enum_AVSampleFormat)(isf), C.int(isr), C.int(lo), unsafe.Pointer(&lc)))
}
